...
fn pngToRGBA(self: *Application, bytes: []const u8) Image {
    _ = self;
...
const World = struct {
    ...
    fn init() World {
        var world = World {
            .entities = std.ArrayList(?*anyopaque).init(Shared.alloc),
            .chunks = Chunks.init(Shared.alloc),
...
const Block = enum {
    ...
    inline fn isSolid(self: *const Block) bool {
        return switch (self.*) {
            .Air => false,
            else => true
        };
